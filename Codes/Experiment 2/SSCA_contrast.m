...
function SSCA_contrast(date,subjectname,start,over)
...
oval_width_change=1;
upordown=[1 -1];
% oval_width1=oval_width+oval_width_change*(upordown(:,randperm(2,1)));
...
%% 光栅相关参数 Grating parameter
 ...
 %% %%%%%%%%%%%%%%%%%%%按任意键开始实验%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   while KbCheck; end
   Screen('DrawText',window,'Contrast task',FixationRect(1)-160,FixationRect(2)-50,[0 0 0]);%提示开始  
...
%             DurationofTest=durationOfTest;
            ...
            if sum(attMatrix(i).att(:,1))>=1
                attStairCaseNum=attMatrix(i).att(1,4);
                if attStairCaseNum==1
                  attChangeAmp=max([min([10^(QuestMean(qAtt_up)) 0.5]) 0.0001]);
                elseif attStairCaseNum==2
                  attChangeAmp=max([min([10^(QuestMean(qAtt_down)) 0.5]) 0.0001]);
                end
                attMatrix(i).att(1,5)=attChangeAmp;
                attStrength=attChangeAmp*randsrc(1,1,[-1 1]);
                contrastChange=0.75+attStrength;
                adaptGratingmatrix_change=flickergratingmatrix(frame_rate,background,FramesofOneFlickerPeriod,o_size1,o_Speriod1,o_flicker1,o_phase1,o_contrast1,o_orientation1,o_blur1,o_TRU1,o_TRD1,...
                                          gap,i_size1,i_Speriod1,i_flicker1,i_phase1,contrastChange,i_orientation1,i_blur1,i_TRU1,i_TRD1);  
                adaptGratingmatrix_change=round(adaptGratingmatrix_change);     
            end
            ...
            if mod(i,TrialperBlock/2)==1%每种条件下第一个trial呈现时间为20s，之后呈现时间为4s
                ...
                for aa=1:40
                    ...
                    for pp=1:round(FramesofOneFlickerPeriod)
                        ...
                        if attMatrix(i).att(n,1) == 1
                            AdaptGratingTexture1(pp)=Screen('MakeTexture',window,adaptGratingmatrix_change(:,:,pp)); 
                            AdaptGratingTexture(pp)=Screen('MakeTexture',window,adaptGratingmatrix(:,:,pp)); 
                            if attMatrix(i).att(n,3)==1
                                Screen('DrawTexture',window,AdaptGratingTexture1(pp),[],adaptGratingRect1(1,:),leftRotation);
                            ...
                            elseif attMatrix(i).att(n,3)==2
                                ...
                                Screen('DrawTexture',window,AdaptGratingTexture1(pp),[],adaptGratingRect1(2,:),rightRotation);
                            end
                        else
                            AdaptGratingTexture(pp)=Screen('MakeTexture',window,adaptGratingmatrix(:,:,pp)); 
                            Screen('DrawTexture',window,AdaptGratingTexture(pp),[],adaptGratingRect1(1,:),leftRotation);
                        ...
                        elseif keycode(qkey)==1
                            if ~dummymode
                                ...
                                status = Eyelink('ReceiveFile',edfFile,[subjectid startnumber '_c_Eyelink.edf']);
                            end
                            sca;
                            return
                        end
...
%                         if attStairCaseNum==1
%                           attChangeAmp=max([min([10^(QuestMean(qAtt_up)) 40]) 0.5]);
%                         elseif attStairCaseNum==2
%                           attChangeAmp=max([min([10^(QuestMean(qAtt_down)) 40]) 0.5]);
%                         end
%                         attMatrix(i).att(n,5)=attChangeAmp;
%                         attStrength=attChangeAmp*attMatrix(i).att(n,2);
%                         if attMatrix(i).att(n,3)==1
%                             leftRotation=attStrength;
%                             rightRotation=0;
%                         elseif attMatrix(i).att(n,3)==2
%                             leftRotation=0;
%                             rightRotation=attStrength;
%                         end
%                     end
                    if attMatrix(i).att(n,7)==1
                        oval_width1=oval_width+oval_width_change*(upordown(:,randperm(2,1)));
...
%                 Beeper(1000,0.7,0.2);
        ...
        %% -------------------------------------------------
        save([subjectid '_c_paramatrix'],'paramatrix','attMatrix');  
