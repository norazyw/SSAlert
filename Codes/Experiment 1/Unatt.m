...
function Unatt(date,subjectname,start,over)
...
if start==1
    ...
    load(['Data/' subjectid  '_u_paramatrix']);  
end
HideCursor;%隐藏鼠标光标
TrakingEyes=eye;
TrialperBlock=0.5*length(paramatrix);%实验分为4个block
 %%%%%%%%%%%%%%%%%%%%%%打开屏幕%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% %%%%%%%%%%%%%%%%%%%基本刺激参数%%%%%%%%%%%%%%%%%%%%%%%%%%%
 %%%%周期
 DistanceToScreen=78;                                  %屏幕距离 厘米
 WidthOfScreen=36.5;                                        %屏幕宽度 厘米
 ...
 fixRange=2;
 xCenter=windowRect(3)/2; %屏幕中心横坐标   
 yCenter=windowRect(4)/2; %屏幕中心纵坐标
 ...
 InterTestBlank=300;%2IFC中间的间隔时间 blank screen between 2IFC
...
FramesofSound=round(duration/framedur);
 %% 椭圆参数 Oval parameter
oval_length=round(0.4*PixelsPerDeg);%椭圆长度
oval_width=round(0.3*PixelsPerDeg);%椭圆宽度
oval_red=[200 50 50];%红色椭圆
oval_black=[50 50 50];%灰色椭圆

ovalrect0_1=[0 0 oval_width oval_length];%椭圆1大小
ovalrect1=CenterRect(ovalrect0_1,windowRect);%%位置

ovalrect0_2=[0 0 oval_length oval_width];%椭圆2大小
ovalrect2=CenterRect(ovalrect0_2,windowRect);%%位置
...
%% 光栅相关参数 Grating parameter
 ...
 %% %%%%%%%%%%%%%%%%%%%按任意键开始实验%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   while KbCheck; end
   Screen('DrawText',window,'Reaction Time Task',FixationRect(1)-140,FixationRect(2)-50,[175 0 0]);%提示开始  
   ...
   KbWait;
    ...
    for mm=start:over
        ...
        %%两条阶梯的初始值
        q_up1=QuestCreate(log10(tGuess(1)),tGuessSd,pThreshold,beta,delta,gamma);% 上升序列
        q_down1=QuestCreate(log10(tGuess(2)),tGuessSd,pThreshold,beta,delta,gamma);% 下降序列
...
%              %初始值
%              maxstrength(1)=0.1;%刺激呈现对比度比率的最大值
%              minstrength(1)=0.02;%刺激呈现对比度比率的最小值
%              initialstrength(1,:)=[minstrength(1) maxstrength(1)];
%             %计算阶梯法α的值
%             if paramatrix(i,4)~=0
%                 StaircaseNum=paramatrix(i,4);  %2,上升序列，1,下降序列
%                 if StaircaseNum==1 %上升和下降序列是不同的阶梯序列
                    ...
                    for pp=1:round(FramesofOneFlickerPeriod)
                        if mod(aa,2)==0 && pp==FramesofResPerPeriod% end and start of one oval
                            if (ovalMatrix(i).oval(n,1) == 1 && kd==1) || (ovalMatrix(i).oval(n,1) ~= 1 && kd==0)    
                                ovalMatrix(i).oval(n,4)=1;%反应正确
                            else
                                ovalMatrix(i).oval(n,4)=-1;%反应错误
                                if kd==0 && probe
                        ...
                        elseif keycode(qkey)==1
                            ...
                            if ~dummymode
                                ...
                                try
                                    ...
                                    filename=['Data/' subjectid startnumber '_u_Eyelink.edf'];
...
%                 Beeper(1000,0.7,0.2);
           ...
           %呈现问题，判断刺激方向
            ...
            if paramatrix(i,3)==paramatrix(i,6)
                ...
                paramatrix(i,7)=0;
                if paramatrix(i,4)~=0
                    errorsound_player=audioplayer(errorsound_data0,samplefrq);%%声音的handle
                    play(errorsound_player);
                end
            end     
...
%             if StaircaseNum==1
%                 responsematrix1=responsematrix;%把此次trial被试的反应结果放到阶梯法下降序列（1）的反应矩阵里去
%             elseif StaircaseNum==2
...
%             end
        ...
        save(['Data/' subjectid '_u_paramatrix'],'paramatrix','ovalMatrix');  
        
        if TrialperBlock>=80
            if i==TrialperBlock/2+(mm-1)*TrialperBlock
                Screen('DrawText',window,'Reaction Time Task',FixationRect(1)-140,FixationRect(2)-50,[175 0 0]);
